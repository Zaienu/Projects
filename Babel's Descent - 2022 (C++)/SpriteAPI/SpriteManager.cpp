...
sf::Vector2f SpriteManager::GetSize(std::string _name)
{
	for (int i = 0; i < m_SpriteVec.size(); i++)
	{
		if (m_SpriteVec[i].name == _name)
		{
			sf::Vector2f size(
				m_SpriteVec[i].sprite.getGlobalBounds().width,
				m_SpriteVec[i].sprite.getGlobalBounds().height);
			return size;
		}
	}
}
...
sf::Color SpriteManager::GetColor(std::string _name)
{
	for (int i = 0; i < m_SpriteVec.size(); i++)
	{
		if (m_SpriteVec[i].name == _name)
		{
			return m_SpriteVec[i].sprite.getColor();
		}
	}
}

void SpriteManager::SetColor(std::string _name, sf::Color _color)
{
	for (int i = 0; i < m_SpriteVec.size(); i++)
	{
		if (m_SpriteVec[i].name == _name)
		{
			return m_SpriteVec[i].sprite.setColor(_color);
		}
	}
}
...
void SpriteManager::BlitSprite(SpriteDrawParameters _parameters)
{
	...
	for (int i = 0; i < m_SpriteVec.size(); i++)
	{
		if (m_SpriteVec[i].name == _parameters.name)
		{
			...
			{
				_parameters.texture->draw(m_SpriteVec[i].sprite, tempState);
			}
		}
	}
}
